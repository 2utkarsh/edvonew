...
export async function GET(request: Request) {
  ...
  const items = await BlogModel.find().populate('author', 'name').sort({ updatedAt: -1 }).lean();
  return toResponse(ok(items.map((item) => ({ ...mapBlogDocumentToPublicBlog(item), status: item.status || 'draft' }))));
}
...
export async function POST(request: Request) {
  ...
  return toResponse(created({ ...mapBlogDocumentToPublicBlog(createdItem), status: createdItem?.status || 'published' }));
}
