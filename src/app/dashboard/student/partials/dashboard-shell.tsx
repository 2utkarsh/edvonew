...
const DashboardShell = ({ children, tab }: { children: ReactNode; tab: string }) => {
   const { screen } = useScreen();
   const [open, setOpen] = useState(false);
   const { props } = usePage<SharedData & { status?: string }>();
   const { translate, auth, status } = props;
   ...
   const tabs = useMemo(
      ...
      [button.courses, button.profile, button.settings, button.wishlist],
   );
