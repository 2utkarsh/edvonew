...
const howItWorks = [
  ...
  { step: '02', title: 'Submit Your Work', description: 'Share your insights and solutions via LinkedIn and publish as a post.' },
  { step: '03', title: 'Get Recognized', description: 'Showcase your work to stand out, earn rewards, and win prizes.' },
];
...
export default function ChallengesPage() {
  ...
  return (
    <main className="min-h-screen bg-slate-50 dark:bg-slate-950">
      ...
      <section className="bg-slate-900 py-24 text-white dark:bg-slate-950 border-t border-slate-800"><div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8"><div className="mb-12 text-center md:text-left"><h2 className="text-3xl font-bold sm:text-4xl text-white">Completed Challenges</h2><p className="mt-4 text-slate-400 text-lg">Solve practical data problems and grow through hands-on experience.</p></div>{isLoading ? <div className="grid gap-6 md:grid-cols-2 lg:grid-cols-3">{Array.from({ length: 3 }).map((_, index) => <div key={index} className="h-[280px] animate-pulse rounded-[2rem] border border-white/10 bg-white/5" />)}</div> : completedChallenges.length ? <div className="grid gap-6 md:grid-cols-2 lg:grid-cols-3">{completedChallenges.map((challenge) => <div key={challenge.id} className="group flex flex-col justify-between rounded-[2rem] border border-white/10 bg-white/5 p-8 transition-all hover:bg-white/10 hover:border-white/20"><div><div className="mb-6 flex items-center gap-3"><History className="h-5 w-5 text-slate-400" /><span className="rounded-full bg-slate-800 px-3 py-1 text-xs font-bold text-slate-300">ARCHIVED</span></div><h3 className="text-lg font-bold leading-relaxed text-white group-hover:text-primary-300 line-clamp-3">{challenge.title}</h3></div><div className="mt-8 border-t border-white/10 pt-6"><div className="flex items-center justify-between text-sm text-slate-400 mb-6"><span className="flex items-center gap-2"><Users2 className="h-4 w-4"/> {challenge.participants} Participated</span><span className="font-bold text-white tracking-wide">{challenge.prize} Prize</span></div><Link href={challenge.href} className="flex w-full items-center justify-center rounded-xl bg-white/10 py-3 text-sm font-bold text-white transition-colors group-hover:bg-primary-600">Practice Now</Link></div></div>)}</div> : <div className="rounded-[2rem] border border-dashed border-white/15 bg-white/5 p-12 text-center text-slate-400">No completed challenges yet.</div>}<div className="mt-16 text-center"><Link href="/challenges/all"><Button variant="outline" size="lg" className="!rounded-full !border-white/20 !text-white hover:!bg-white/10 px-8">View All Challenges</Button></Link></div></div></section>
    </main>
  );
}
