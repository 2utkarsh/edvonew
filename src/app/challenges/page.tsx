...
export default function ChallengesPage() {
  ...
  const hasLiveChallenges = ongoingChallenges.length > 0;
  const featuredChallenges = hasLiveChallenges ? ongoingChallenges : completedChallenges;
  ...
  return (
    <main className="min-h-screen bg-slate-50 dark:bg-slate-950">
      ...
      <section id="ongoing-challenges" className="relative mx-auto max-w-7xl px-4 py-20 sm:px-6 lg:px-8">
        <div className="mb-12"><Badge variant="accent" className="mb-3 uppercase tracking-wider">{hasLiveChallenges ? 'Live Now' : 'Practice Now'}</Badge><h2 className="text-3xl font-bold text-slate-900 dark:text-white sm:text-4xl">{hasLiveChallenges ? 'Ongoing Resume Project Challenge(s)' : 'Featured Resume Project Challenge(s)'}</h2><p className="mt-3 text-lg text-slate-600 dark:text-slate-400">{hasLiveChallenges ? 'Open a competition page, review the brief, and start building your submission.' : 'No live challenge is running right now, so we are showing the latest challenge pages added from admin.'}</p></div>
