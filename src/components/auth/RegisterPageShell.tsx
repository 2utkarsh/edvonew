...
const COPY = {
  student: {
    ...
    pageTitle: 'Register',
    pageBody: 'Create your EDVO learner account to access courses, events, and community updates.',
    loginHref: '/auth/login',
    registerButton: 'Create Account',
  },
  instructor: {
    role: 'instructor',
    heroTitle: 'Teach With EDVO',
    heroBody:
    ...
    pageBody: 'Complete this form to create your instructor account and access the EDVO teaching workspace.',
    loginHref: '/instructor',
    registerButton: 'Create Instructor Account',
  },
} as const;

export default function RegisterPageShell({ variant }: RegisterPageShellProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setTimeout(() => router.push(copy.loginHref), 500);
    } catch (error: any) {
        ...
        <div className="mx-auto flex min-h-[calc(100vh-64px)] max-w-7xl overflow-hidden lg:rounded-[2rem] lg:border lg:border-slate-200/70 lg:bg-white/70 lg:shadow-xl lg:shadow-slate-200/40 lg:backdrop-blur dark:lg:border-slate-800 dark:lg:bg-slate-950/60 dark:lg:shadow-black/20">
          <div className="hidden lg:flex lg:w-[45%] xl:w-[42%] flex-col items-center justify-center relative overflow-hidden bg-gradient-to-br from-slate-50 to-blue-50/50 dark:from-slate-900 dark:to-primary-950/50 px-12 border-r border-gray-100 dark:border-slate-800 transition-colors">
            ...
            <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.6 }} className="relative z-10 text-center">
              ...
              <h2 className="text-2xl font-bold text-gray-800 dark:text-white mb-2">{copy.heroTitle}</h2>
              <div className="my-8">
                <Image src="/images/student-illustration.png" alt="EDVO illustration" width={280} height={280} className="mx-auto drop-shadow-lg" priority />
              </div>
              <p className="text-gray-500 dark:text-gray-400 text-sm max-w-xs mx-auto leading-relaxed">{copy.heroBody}</p>
            </motion.div>
          </div>
          ...
          <div className="flex-1 flex items-center justify-center px-6 py-10 lg:px-12">
            <motion.div initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} transition={{ duration: 0.5, delay: 0.1 }} className="w-full max-w-md">
              <div className="flex items-center justify-between mb-2">
                ...
                <p className="text-sm text-gray-500 dark:text-gray-400">Have an Account? <Link href={copy.loginHref} className="text-primary-600 dark:text-primary-400 font-semibold hover:underline">Log in</Link></p>
              </div>

              <div className="mb-6">
                <h1 className="mt-4 text-3xl font-bold text-gray-900 dark:text-white">{copy.pageTitle}</h1>
                <p className="mt-2 text-sm text-slate-500 dark:text-slate-400">{copy.pageBody}</p>
              </div>
              ...
              <form onSubmit={handleSubmit} className="space-y-4">
                ...
                <button type="submit" disabled={isLoading} className="w-full py-3 bg-gradient-to-r from-primary-600 to-primary-700 text-white font-semibold rounded-lg hover:from-primary-700 hover:to-primary-800 transition-all duration-300 disabled:opacity-60 disabled:cursor-not-allowed shadow-md hover:shadow-lg">
                  {isLoading ? <span className="flex items-center justify-center gap-2"><span className="w-4 h-4 border-2 border-white/30 border-t-white rounded-full animate-spin" />Creating Account...</span> : copy.registerButton}
                </button>
