...
const COPY: Record<PortalVariant, LoginCopy> = {
  student: {
    eyebrow: 'Welcome to',
    title: 'Log in',
  ...
  instructor: {
    ...
    heroBody:
      'Access your EDVO teaching workspace to manage programs, host live sessions, and support learners.',
    submitLabel: 'Enter Instructor Portal',
  },
} as const;

export default function LoginPageShell({ variant }: LoginPageShellProps) {
  ...
  const validatePortalRole = (role?: string) => {
    if (variant === 'student' && role === 'instructor') {
      throw new Error('Instructor accounts must sign in through edvo.in/instructor.');
    }

    if (variant === 'student' && role === 'admin') {
      throw new Error('Admin accounts must sign in through the admin portal.');
    }

    if (variant === 'instructor' && role === 'student') {
      throw new Error('Student accounts must sign in through the student login page.');
    }

    if (variant === 'instructor' && role === 'admin') {
      throw new Error('Admin accounts must sign in through the admin portal.');
    }
  };
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      validatePortalRole(payload?.user?.role);
        ...
        <div className={`mx-auto flex max-w-7xl overflow-hidden ${variant === 'student' ? 'min-h-[calc(100vh-64px)] lg:rounded-[2rem] lg:border lg:border-slate-200/70 lg:bg-white/70 lg:shadow-xl lg:shadow-slate-200/40 lg:backdrop-blur dark:lg:border-slate-800 dark:lg:bg-slate-950/60 dark:lg:shadow-black/20' : 'min-h-screen'}`}>
          <div className="hidden lg:flex lg:w-[45%] xl:w-[42%] flex-col items-center justify-center relative overflow-hidden bg-gradient-to-br from-slate-50 to-blue-50/50 dark:from-slate-900 dark:to-primary-950/50 px-12 border-r border-gray-100 dark:border-slate-800 transition-colors">
            ...
            >
              ...
              {copy.heroTitle ? <h2 className="text-2xl font-bold text-gray-800 dark:text-white mb-2">{copy.heroTitle}</h2> : null}
            ...
            >
              <div className="flex items-center justify-between mb-2 gap-4">
                <p className="text-gray-600 dark:text-gray-400">
                  {copy.eyebrow} <span className="text-primary-600 dark:text-primary-400 font-semibold">EDVO</span>
                </p>
                {'registerHref' in copy && copy.registerHref && 'registerLabel' in copy && copy.registerLabel ? (
                  <p className="text-sm text-gray-500 dark:text-gray-400 text-right">
                    No Account?{' '}
                    <Link href={copy.registerHref} className="text-primary-600 dark:text-primary-400 font-semibold hover:underline">
                      {copy.registerLabel}
                    </Link>
                  </p>
                ) : null}
              ...
              <div className="mb-8">
                ...
                {'switchPrompt' in copy && copy.switchPrompt && 'switchHref' in copy && copy.switchHref && 'switchLabel' in copy && copy.switchLabel ? (
                  <p className="mt-2 text-sm text-slate-500 dark:text-slate-400">
                    {copy.switchPrompt}{' '}
                    <Link href={copy.switchHref} className="font-semibold text-primary-600 dark:text-primary-400 hover:underline">
                      {copy.switchLabel}
                    </Link>
                  </p>
                ) : null}
