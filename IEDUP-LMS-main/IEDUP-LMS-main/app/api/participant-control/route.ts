...
export async function POST(req: Request) {
  try {
    ...
    try {
      if (action === 'record-face-reference' || action === 'face-verification') {
        const existingMetadata = safeParseMetadata(participant.metadata);
        const existingFaceVerification = existingMetadata.faceVerification || {};
        const checkedAt = new Date().toISOString();
        const history = Array.isArray(existingFaceVerification.history)
          ? existingFaceVerification.history.slice(-19)
          : [];
          ...
          faceVerification: {
            ...
            intervalMinutes:
              typeof body.intervalMinutes === 'number'
                ...
                : existingFaceVerification.intervalMinutes || null,
            history,
          },
        };
